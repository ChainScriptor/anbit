...
const OrderDeliveredScreen: React.FC<OrderDeliveredScreenProps> = ({
  pin,
  tableNumber = 1,
  xpEarned = 0,
  partnerName,
    ...
    >
      ...
      <main className="mx-auto w-full max-w-2xl flex-1 space-y-8 overflow-y-auto px-6 pb-[calc(8.35rem+env(safe-area-inset-bottom))] pt-6">
        <section className="space-y-4 py-2 text-center">
          ...
          <p className="anbit-wordmark px-1 text-3xl tracking-tight text-[#0a0a0a] sm:text-4xl">
            {t('orderDeliveredTitle')}
          </p>
          <p className="text-base font-medium text-[#0a0a0a]/60">{t('orderDeliveredSubtitle')}</p>
        </section>

        <section className="flex flex-col items-center justify-center space-y-2 rounded-2xl border border-white/10 bg-[#0a0a0a] p-8">
          <p className="text-xs font-bold uppercase tracking-[0.3em] text-white/45">{t('servingToTable')}</p>
          <div className="text-5xl font-black tracking-tighter text-white sm:text-6xl">B-{tableNumber}</div>
        ...
        <section className="relative px-2 py-6 sm:px-4">
          <div className="relative mx-auto flex max-w-sm justify-between">
            ...
            <div className="absolute left-0 top-5 z-0 h-0.5 w-full rounded-full bg-[#0a0a0a]/12" aria-hidden />
            {/* Progress line active */}
            <div className="absolute left-0 top-5 z-0 h-0.5 w-full rounded-full" style={{ backgroundColor: BRAND_RED }} aria-hidden />
            ...
            <div className="relative z-10 flex flex-col items-center gap-3">
              <div
                className="flex h-10 w-10 items-center justify-center rounded-full border-4 border-[#ffffff] bg-[#0a0a0a]/35 shadow-sm"
                style={{ backgroundColor: BRAND_RED }}
              >
                <ConciergeBell className="h-4 w-4 text-white" strokeWidth={2} />
              </div>
              <span className="max-w-[4.5rem] whitespace-pre-line text-center text-[10px] font-bold uppercase leading-tight tracking-tight text-[#0a0a0a]">
            ...
            <div className="shrink-0 text-right">
              ...
              {xpEarned > 0 && (
                <div className="mt-1 flex items-center justify-end gap-1" style={{ color: XP_GOLD }}>
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-wider">+{xpEarned} {t('xpEarnedShort')}</span>
                </div>
              )}
            </div>
          </div>
          <div className="border-t border-white/10 pt-4">
            <p className="mb-4 text-[10px] font-bold uppercase tracking-widest text-white/45">{t('yourSelection')}</p>
              ...
              {lines.length === 0 ? (
                ...
                lines.map((line, i) => (
                  <div key={`${line.name}-${i}`} className="flex items-center justify-between gap-3 text-sm">
                    ...
                    <span className="shrink-0 font-medium text-white">€{(line.unitPrice * line.quantity).toFixed(2)}</span>
                  </div>
                ))
              )}
            </div>
        ...
        <div className="py-2 text-center">
          <button type="button" className="text-xs font-bold uppercase tracking-[0.2em] text-[#0a0a0a]/45 transition-colors hover:text-[#0a0a0a]">
