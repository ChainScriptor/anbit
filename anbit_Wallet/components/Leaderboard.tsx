        ...
        {entries.slice(0, 5).map((user) => (
          <div key={user.rank} className="flex items-center justify-between group cursor-pointer transition-all hover:translate-x-2 sm:hover:translate-x-4">
            <div className="flex items-center gap-4 sm:gap-10">
              <span className={`text-2xl sm:text-5xl font-black w-8 sm:w-14 text-center italic ${user.rank <= 3 ? 'text-anbit-xp-accent scale-110 drop-shadow-xl' : 'text-anbit-muted opacity-30'}`}>
                {user.rank < 10 ? `0${user.rank}` : user.rank}
              </span>
              <div className="flex flex-col min-w-0">
                <div className="flex items-center gap-2 sm:gap-4">
                  <span className={`text-lg sm:text-3xl font-bold tracking-tighter truncate normal-case ${user.isCurrentUser ? 'text-anbit-xp-accent' : 'text-anbit-text'}`}>
                    {user.name}
                  </span>
                  {user.isCurrentUser && (
