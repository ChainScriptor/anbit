...
/**
 * Απαλά μαύρα κύματα που «χωνεύονται» στο anbit-bg — χωρίς αίσθηση πλαισίου.
 */
export const QuickCategoriesWaveBackdrop: React.FC = () => (
  <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden style={edgeMask}>
    ...
    <motion.div
      className="absolute -bottom-10 left-[-32%] w-[164%] opacity-[0.55]"
      style={{ height: '48%', minHeight: 88 }}
      initial={false}
      animate={{ x: ['0%', '-3.5%', '0%'] }}
      transition={{ duration: 16, repeat: Infinity, ease: 'easeInOut' }}
    >
      <svg viewBox="0 0 1440 240" className="h-full w-full text-black/35" preserveAspectRatio="none">
        <path
          fill="currentColor"
    ...
    <motion.div
      className="absolute -bottom-8 left-[-38%] w-[176%] opacity-[0.45]"
      style={{ height: '42%', minHeight: 76 }}
      initial={false}
      animate={{ x: ['0%', '4%', '0%'] }}
      transition={{ duration: 20, repeat: Infinity, ease: 'easeInOut' }}
    >
      <svg viewBox="0 0 1440 220" className="h-full w-full text-black/40" preserveAspectRatio="none">
        <path
          fill="currentColor"
    ...
    <motion.div
      className="absolute -bottom-6 left-[-28%] w-[156%] opacity-[0.35]"
      style={{ height: '36%', minHeight: 64 }}
      initial={false}
      animate={{ x: ['0%', '-2.5%', '0%'] }}
      transition={{ duration: 24, repeat: Infinity, ease: 'easeInOut' }}
    >
      <svg viewBox="0 0 1440 200" className="h-full w-full text-black/30" preserveAspectRatio="none">
        <path
          fill="currentColor"
